...
while (!exit)
		...
		if (num == -1)
			...
			exit = true
				...
				while (!plus && !exit)
				{
				print "Please introduce a consecutive number (type '-1' to end programme) minus detected: "
				str = System.console().readLine()
				num = Integer.parseInt(str)
					if (num == -1)
						{
							...
							while (plus && !exit)
							{
								print "Please introduce a consecutive number (type '-1' to end programme) plus dectected: "
								str = System.console().readLine()
								num = Integer.parseInt(str)
								if (num == -1)
									{
